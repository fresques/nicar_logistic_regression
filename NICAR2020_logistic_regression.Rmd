...
We know that we can use a binary explanatory variable in a linear regression model. But what if our result variable is binary?
```{r}
...
print(m.linear)
```
if we plug 70 percent into our linear equation taken from our regression coefficients, then the probability of getting a tick for driving 70 percent above the speed limit, then the probability of getting a ticket is r -0.326162 + 0.019792 * 70
```{r}
-0.326162 + 0.019792 * 70
```
